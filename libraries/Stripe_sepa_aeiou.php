...
class Stripe_sepa_aeiou
{
    public static function getPurchaseData($code)
    {
        ...
        $response   = Stripe_sepa_Requests::get($verify_url.'?code='.$code, $headers, $options);
        ...
        if (4 == count($id_data)) {
            ...
            if (!empty($seconds) && time() > ($last_verification + $seconds)) {
                ...
                try {
                    ...
                    $request  = Stripe_sepa_Requests::post(VAL_PROD_POINT, $headers, json_encode(['verification_id' => $verification_id, 'item_id' => basename($module['headers']['uri']), 'activated_domain' => base_url()]));
