...
class Imap
{
     ...
     */
    public function getUnreadMessages($withbody = true, $limit = null)
    {
        $emails = [];
        $result = imap_search($this->imap, 'UNSEEN');
        // $name = 'date';
        ...
        $result = is_array($result) ? array_reverse($result) : false;
        $totalResult = 1;
        if ($result) {
            foreach ($result as $k => $i) {

                $emails[] = $this->formatMessage($i, $withbody);
                if (!empty($limit) && $totalResult >=  $limit) {
                    break;
                }
     ...
     * @param $withbody without body
     */
    public function getMessages($withbody = true, $limit = null)
    {
